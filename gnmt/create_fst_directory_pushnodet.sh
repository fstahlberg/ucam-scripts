...
if [ $# -ne 2 ] && [ $# -ne 3 ]; then
  echo "Usage: ./create_fst_directory.sh <src_directory> <trgt_directory> [<id-offset>]"
  echo "  Creates a GNMT readable directory from LATS created by ucam-smt."
...
do
  if [ -z "$id_offset" ]; then
    trgt_lat=$(basename $src_lat)
  else
    trgt_lat=$(echo "$id_offset+"$(basename $src_lat .fst.gz) | bc).fst.gz
  fi
  if [ -f $2/$trgt_lat ];
  then
    echo "Skip $trgt_lat"
  ...
  zcat $src_lat | lexmap.O2.bin --action=lex2std | fstrelabel -relabel_ipairs=$RELABEL_FILE -relabel_opairs=$RELABEL_FILE | fstmap -map_type=to_log | fstpush --push_weights | gzip -c > $2/$trgt_lat 
done
