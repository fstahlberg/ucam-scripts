...
do
  ...
  zcat $src_lat | fstprint | awk -v CONVFMT=%.10g '{if (NF>=5) {$5=split($5,a,","); $5=a[1]-a[2];} print $0}' |  fstcompile | fstrelabel -relabel_ipairs=$RELABEL_FILE -relabel_opairs=$RELABEL_FILE | fstrmepsilon | fstdeterminize | fstminimize | fstmap -map_type=to_log | fstpush --push_weights > $2/$trgt_lat 
done
