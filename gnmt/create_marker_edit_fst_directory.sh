...
MARKER_START_ID=800000000

if [ $# -ne 6 ] && [ $# -ne 7 ] && [ $# -ne 8 ]; then
  echo "Usage: ./create_marker_edit_fst_directory.sh <hifst_directory> <nmt_directory> <trgt_directory> <range> <nmt-vocab> <nmt-scale>,<edit-cost>,<ins-unk-cost>,<unk-to-in-vocab>,<hifst-scale> [out-n-best [max-unk-mult]]"
...
# Create UNK replacement FST with Markers
rm $TMP_UNK_TXT
if [ -z "$max_unk_mult" ]; then
  # Unlimited number of unk multiplications
  echo "0 1 $MARKER_START_ID 0" >> $TMP_UNK_TXT
  echo "1 2 $UNK_ID $UNK_ID" >> $TMP_UNK_TXT
  echo "2 2 $UNK_ID $UNK_ID 0,3,1.0" >> $TMP_UNK_TXT
  echo "2 3 $MARKER_START_ID 0" >> $TMP_UNK_TXT
  echo "3" >> $TMP_UNK_TXT
else
  ...
  echo "$prev_state $max_unk_mult $UNK_ID $UNK_ID" >> $TMP_UNK_TXT
  final=$(echo "$max_unk_mult+1" | bc)
  echo "$max_unk_mult $final $MARKER_START_ID 0" >> $TMP_UNK_TXT
  echo "$final" >> $TMP_UNK_TXT
  # Allow unk deletion
...
do
  ...
  # Here we also make the marker labels unique
  fstreplace --epsilon_on_replace $TMP_NMT_PRE $ROOT_ID $TMP_UNK $REPLACE_ID | fstprint | awk '{if ($3 >= 800000000 && $3 < 900000000) {n += 1; $3 = 800000000 + n; } print $0}' | fstcompile --arc_type=tropicalsparsetuple | fstrmepsilon | fstarcsort --sort_type=olabel > $TMP_NMT
  fstprint $TMP_HIFST | cut -f3 | sort -g -u > $TMP_VOCAB_HIFST
  # NMT vocab without UNK and without markers
  fstprint $TMP_NMT | cut -f3 | sort -g -u | egrep -v "^$UNK_ID$" | egrep -v '^8[0-9]{8}' > $TMP_VOCAB_NMT
  ...
  # Compose to final FST, connect marker arcs with connections to a unique marker state
  fstcompose $TMP_NMT $TMP_EDIT | fstcompose - $TMP_HIFST | fstprint | awk '{if ($3 >= 800000000 && $3 < 900000000) { to_state = $2; marker = $3; $2 = marker; $3 = 0; print $0"\n"marker" "to_state" 0 0" } else { print $0 }}' | fstcompile --arc_type=tropicalsparsetuple | gzip -c > $TMP_COMPOSED
  if [ -z "$out_n_best" ]; then
    cp $TMP_COMPOSED $trgt_dir/$id.fst.gz
  else
    # We store it first in TMP_HIFST because we do not want the output file to show up to early
    zcat $TMP_COMPOSED | fstshortestpath --nshortest=$out_n_best | gzip -c > $TMP_HIFST
