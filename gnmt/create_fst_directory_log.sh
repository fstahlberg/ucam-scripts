...
do
  ...
  zcat $src_lat | lexmap.O2.bin --action=lex2std | fstrelabel -relabel_ipairs=$RELABEL_FILE -relabel_opairs=$RELABEL_FILE | fstrmepsilon | fstmap -map_type=to_log | fstdeterminize | fstminimize | fstpush --push_weights > $2/$trgt_lat 
done
