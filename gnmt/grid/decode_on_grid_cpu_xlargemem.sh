...
ID=$(qsub -N gnmt-worker -pe memory_hog 2 -l 'not_host=air124,mem_free=27G,mem_grab=17G,osrel=*' -o $output_dir/logs -e $output_dir/logs -t 1-$num_parts -v config_file=$config_file,num_parts=$num_parts,total_range=$total_range,output_dir=$output_dir $worker_script | awk '{print $3}' | sed 's:\..*::')
HT="-hold_jid $ID"
