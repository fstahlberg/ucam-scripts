...
config_file=$3
output_dir=$4
worker_script=$(dirname $0)/tf_decode_on_grid_cpu_worker.sh
combination_script=$(dirname $0)/decode_on_grid_cpu_combination.sh

mkdir -p $output_dir/logs

# Start workers
ID=$(qsub -N gnmt-worker -pe memory_hog 2 -l 'not_host=(air093|air094|air095|air096|air097|air098|air100|air101),mem_free=1.5G,mem_grab=1.5G,osrel=(12.04|14.04),ubuntu=1' -o $output_dir/logs -e $output_dir/logs -t 1-$num_parts -v config_file=$config_file,num_parts=$num_parts,total_range=$total_range,output_dir=$output_dir $worker_script | awk '{print $3}' | sed 's:\..*::')
HT="-hold_jid $ID"
